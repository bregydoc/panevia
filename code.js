...
figma.ui.onmessage = (msg) => {
    if (msg.type === "create-rectangles") {
        ...
        const palleteWeights = [
            [hexColorToRGB("#FFFFFF"), 0.3],
            [hexColorToRGB("#FD936A"), 0.186667],
        ...
        renderQuads(quads, styleWeights, palleteWeights, maskColor, strokeColor, true, 0.65);
...
const renderQuads = (quads, styleWeights, palleteWeights, maskColor, strokeColor, onlyOutline = false, density = 1) => {
    const nodes = [];
    const maskNodes = [];
    quads.forEach((quad) => {
        ...
        const color = pick(palleteWeights);
        rect.fills = [
            {
                type: "SOLID",
                color: color,
...
const randomColsAndRows = (noise, l, cols, rows, seed) => {
    let columnsArray = [];
    let rowsArray = [];
...
const classicDistribution = (l, cols, rows) => {
    let columnsArray = [];
    let rowsArray = [];
    for (let c = 0; c < cols; c++) {
...
const generateQuads = (cols, rows, start = [0, 0]) => {
    let currentPoint = [start[0], start[1]];
    let quads = [];
    for (let x = 0; x < cols.length; x++) {
        currentPoint = [currentPoint[0], start[1]];
        for (let y = 0; y < rows.length; y++) {
            const p1 = [currentPoint[0], currentPoint[1]];
            const p2 = [currentPoint[0], currentPoint[1] + rows[y]];
            const p3 = [currentPoint[0] + cols[x], currentPoint[1]];
            const p4 = [currentPoint[0] + cols[x], currentPoint[1] + rows[y]];
